...
A regressão linear usando uma única variável independente contínua se torna exatamente uma correlação^[usando a técnica de Pearson.]. Agora quando empregamos mais de uma variável independente, a interpretação da regressão se torna: **"O efeito de `X` em `Y` mantendo `Z` fixo"**. Isto quer dizer que a regressão linear **controla os efeitos das diferentes variáveis independentes ao calcular o efeito de uma certa variável independente**. Esta é o que chamamos de **interpretação estatística** da regressão linear.
...
Para exemplificar as regressões nesse tutorial, usaremos o *dataset* já incluído no R `mtcars` [@henderson1981building]. `mtcars` é uma base de dados extraída da revista americana sobre carros *Motor Trend US* de 1974. Possui 32 observações de carros e 11 variáveis:
