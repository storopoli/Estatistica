...
```{r glm-simples}
modelo_simples <- glm(survived ~ age + sex, data = TitanicSurvival, family = binomial)
...
* `sexmale` -- A cada aumento de uma unidade de idade (ou seja sexomale é igual 1, em outras palavras sexo masculino), há uma dimunição da chance de sobrevivência (variável dependente `survived` igual a 1) em `r (1 - exp(coef(modelo_simples))["sexmale"]) * 100`% ($1 - \text{OR}_\text{sexmale}$).
...
Note que o $p$-valor do Teste de Breusch-Pagan para o `modelo_simples` é menor que 0.05, demonstrando fortes evidências que os resíduos não são independentes.
...
Note que o $p$-valor do Teste de Durbin-Watson para o `modelo_simples` é menor que 0.05, demonstrando fortes evidências que os resíduos são correlacionados entre si, violando o pressuposto da homogeneidade de variância dos resíduos.
